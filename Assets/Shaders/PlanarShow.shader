Shader "Unlit/PlanarShadow"
{
    Properties
    {
        ...
        _ShadowPlane ("ShadowPlane", Vector) = (0.0, 1.0, 0.0, 0.1)
        _ShadowFadeParams ("ShadowFadeParams", Vector) = (0.0, 1.0, 1.0, 0.1)
    }
    SubShader
    {
        ...
        Pass
        {
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				// 1. 王者荣耀的衰减公式
				color.w = pow(
                    1.0 - saturate(distance(i.xlv_TEXCOORD0, i.xlv_TEXCOORD1) * _ShadowFadeParams.w - _ShadowFadeParams.x),
                    _ShadowFadeParams.y
                    ) * _ShadowFadeParams.z;

				// 2. https://zhuanlan.zhihu.com/p/31504088 这篇文章介绍的另外的阴影衰减公式
				// color.w = 1.0 - saturate(distance(i.xlv_TEXCOORD0, i.xlv_TEXCOORD1) * _ShadowFadeParams.w);
